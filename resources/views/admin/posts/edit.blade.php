...
<div class="bg-white shadow-sm rounded-xl p-5">
    ...
    <div class="mb-4">
        ...
        @if ($media)
            ...
            @else
                <video class="w-48 h-48 rounded-lg border" controls>
                    <source src="{{ asset('storage/' . $media->file_path) }}" type="video/mp4">
                </video>
            @endif
        @else
            <p class="text-slate-400 text-sm italic">Tidak ada media</p>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('admin.posts.update', $post->id) }}">
        ...
        <textarea name="caption" rows="4"
                  class="w-full p-3 border rounded-lg bg-slate-50 focus:ring-pink-400 focus:bg-white">{{ $post->caption }}</textarea>

        <div class="mt-4 flex gap-3">
            <button class="px-4 py-2 bg-pink-500 text-white rounded-full text-sm hover:bg-pink-600 transition">
                Simpan Perubahan
            </button>
